...
namespace p2o
{
    ...
    // Pose2D
    Pose2D::Pose2D() : x(0), y(0), th(0) {}
    Pose2D::Pose2D(p2o_float_t in_x, p2o_float_t in_y, p2o_float_t in_t) : x(in_x), y(in_y), th(in_t) {}
    Vec3D Pose2D::vec() const
    {
        Vec3D v;
        v << x, y, th;
        return v;
    }
    Pose2D Pose2D::oplus(const Pose2D r) const
    {
        Vec3D v = rotMat2D(th) * r.vec() + vec();
        return Pose2D(v[0], v[1], v[2]);
    }
    Pose2D Pose2D::ominus(const Pose2D r) const
    {
        Vec3D diff;
    ...
    // Con2D
    void Con2D::setCovarianceMatrix(const Mat3D &mat)
    {
        info = mat.inverse();
    }

    Vec3D ErrorFunc2D::error_func(const Pose2D &pa, const Pose2D &pb, const Pose2D &con)
    {
    ...
    Vec3D ErrorFunc2D::calcError(const Pose2D &pa, const Pose2D &pb, const Pose2D &con, Mat3D &Ja, Mat3D &Jb)
    {
    ...
    // Optimizer2D
    Optimizer2D::Optimizer2D() : verbose(false), lambda(0), stop_thre(1e-3), robust_delta(std::numeric_limits<double>::max()) {}
    Optimizer2D::~Optimizer2D() {}
    void Optimizer2D::setLambda(double val)
    {
        lambda = val;
    }

    void Optimizer2D::setStopEpsilon(double val)
    {
        stop_thre = val;
    }

    void Optimizer2D::setRobustThreshold(double val)
    {
        robust_delta = val;
    }

    void Optimizer2D::setVerbose(bool val)
    {
        verbose = val;
    }
    ...
    // RotVec
    RotVec::RotVec() : ax(0), ay(0), az(0) {}
    RotVec::RotVec(p2o_float_t x, p2o_float_t y, p2o_float_t z) : ax(x), ay(y), az(z) {}
    RotVec::RotVec(const Eigen::Quaterniond &in_q)
    {
    ...
    p2o_float_t RotVec::norm() const
    {
        return sqrt(ax*ax + ay*ay + az*az);
    }

    RotVec RotVec::inverted() const
    {
        return RotVec(-ax, -ay, -az);
    }

    Eigen::Quaterniond RotVec::toQuaternion() const
    {
    ...
    Mat3D RotVec::toRotationMatrix() const
    {
        return toQuaternion().toRotationMatrix();
    }

    Eigen::Matrix<p2o_float_t, 4, 3> dQuat_dRV(const RotVec &rv)
    {
    ...
    void dR_dRV(const RotVec &rv, Mat3D &dux, Mat3D &duy, Mat3D &duz)
    {
    ...
    Eigen::Matrix<p2o_float_t, 3, 4> dRV_dQuat(const Eigen::Quaterniond &q)
    {
        if (1-q.w()*q.w() < 1e-7)
        {
    ...
    // Pose3D
    Pose3D::Pose3D() : x(0), y(0), z(0) {}
    Pose3D::Pose3D(p2o_float_t in_x, p2o_float_t in_y, p2o_float_t in_z, const RotVec &in_rv) : x(in_x), y(in_y), z(in_z), rv(in_rv) {}
    Pose3D::Pose3D(p2o_float_t in_x,  p2o_float_t in_y,  p2o_float_t in_z,
           p2o_float_t ax, p2o_float_t ay, p2o_float_t az) : x(in_x), y(in_y), z(in_z), rv(ax, ay, az) {}

    Vec6D Pose3D::vec() const
    {
        Vec6D v;
    ...
    Pose3D Pose3D::oplus(const Pose3D &rel) const
    {
    ...
    Pose3D Pose3D::ominus(const Pose3D &base) const
    {
    ...
    //ErrorFunc3D
    Vec6D ErrorFunc3D::error_func(const Pose3D &pa, const Pose3D &pb, const Pose3D &con)
    {
    ...
    Vec6D ErrorFunc3D::calcError(const Pose3D &pa, const Pose3D &pb, const Pose3D &con, Mat6D &Ja, Mat6D &Jb)
    {
    ...
    Optimizer3D::Optimizer3D() : verbose(false), lambda(0), stop_thre(1e-3), robust_delta(std::numeric_limits<double>::max()) {}
    Optimizer3D::~Optimizer3D() {}
    void Optimizer3D::setLambda(double val)
    {
        lambda = val;
    }

    void Optimizer3D::setStopEpsilon(double val)
    {
        stop_thre = val;
    }

    void Optimizer3D::setRobustThreshold(double val)
    {
        robust_delta = val;
    }

    void Optimizer3D::setVerbose(bool val)
    {
        verbose = val;
    }
